    ...
    def process_file(self):
        ...
        if is_image and num_cards > 0:
            ...
            for idx, (x, y, w, h) in enumerate(faces):
                ...
                try:
                    print("About to extract from:", self.uploaded_file)
                    print("extract_id_info got path:", self.uploaded_file)
                    info = extract_id_info(temp_path)
                    print("OCR TEXT:", info['Raw Text'])
                    infos.append(info)
                except Exception as e:
        ...
        for idx, info in enumerate(infos):
            # Format the extracted info
            output_lines = [
                f"Detected ID Type: {info['ID Type']}",
                "--------------------------------",
                "Extracted Details:"
            ]
            ...
            # Display in a Label (monospaced font for clarity)
            ...
            # Show Raw Text section
            raw_text_title = tk.Label(self.results_frame, text="Full OCR Text:", font=("Arial", 11, "bold"), anchor="w", justify="left")
            raw_text_title.pack(anchor=tk.W, pady=(2, 0))
            self.results_widgets.append(raw_text_title)
            raw_text_label = tk.Label(self.results_frame, text=info['Raw Text'], font=("Consolas", 10), justify="left", anchor="w", wraplength=700, bg="#f4f4f4")
            raw_text_label.pack(anchor=tk.W, pady=(0, 10))
            self.results_widgets.append(raw_text_label)
